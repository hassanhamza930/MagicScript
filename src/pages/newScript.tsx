...
function NewScript() {
    ...
    const [scriptLines, setscriptLines] = useState([{text:""}] as Array<ScriptLine>);
            ...
            <div className="relative h-full w-full flex flex-col justify-start items-start">
                ...
                <div className="w-full flex flex-row justify-between items-center absolute z-10">
                    <form onSubmit={(e) => {
                        ...
                        {
                            isEditingName == true ?
                                <input value={scriptName} onChange={(e) => { setscriptName(e.target.value) }} type="text" className="w-96 bg-transparent border-b-[1px] pb-1 border-white outline-none text-white/80 text-2xl font-medium" />
                                :
                        ...
                        {
                            isEditingName == true ?
                                <Button type="submit" className="hover:scale-105 bg-white/90 transiton-all duration-300 tracking-normal ">
                                    Save
                                </Button>
                                :
                    ...
                    <Button disabled={scriptLines[0].text!=""?false:true } onClick={()=>{publish();}} className="hover:scale-105 bg-[#f4f269] shadow-[#5cb270] shadow-sm backdrop-blur-md rounded-sm transiton-all duration-300 tracking-normal mr-10 ">
                        Publish
                    </Button>
                ...
                <div className="tracking-normal relative mt-14 flex flex-col justify-start items-center h-full bg-white/10 rounded-xl p-2 px-4 overflow-y-auto">
                            ...
                            return (
                                <div className="flex flex-col justify-start items-center">
                                    <textarea value={line.text} onChange={(e) => {
                                        var newLines = scriptLines;
                                        newLines[index].text = e.target.value;
                                        setscriptLines([...newLines]);
                                    }} placeholder="Hello John, how are you doing today..." className="w-96 h-24 text-sm rounded-xl border-[2px] border-white/60 px-4 py-2 bg-black/20 text-white">
                                    </textarea>
